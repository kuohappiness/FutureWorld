...
- `Tools\Open-FWMVPCombatGaragePIE.ps1 -StartPIE -VehicleDestructionEvidence -VehicleDestructionTarget Car|Motorcycle` writes a `*_state-evidence.jsonl` file for `PIE-13`, `PIE-14`, and `PIE-15` with before-destruction health/state, after-destruction `State.Vehicle.Destroyed`, dead health, registered core-vehicle flag, PlayerState/HUD lives, core vehicle health, match state, respawn event count, player pawn/health/location, and delegate/gameplay event counts that must remain `1` after a repeated destroy attempt. Add `-VehicleDestructionEndMatchEvidence` for `PIE-16`; it records the zero-lives path from PlayerState/HUD lives `1` to `0`, `Match.Ended`, no respawn event, and stable ended-state presentation. The sequence advances on PIE world time so respawn/end-match evidence is sampled after gameplay timers can complete.
- `Tools\Open-FWMVPCombatGaragePIE.ps1 -StartPIE -AIPresentationEvidence` writes a `*_state-evidence.jsonl` file for `PIE-18` with the selected AI, HUD AI counts, HUD nearest-AI state, Alert pursuit distance, AttackOnFoot distance, state-change count, weapon/damage event counts, and player health after attack. The acceptance evidence should prove `State.AI.Alert` outside attack range, `State.AI.AttackOnFoot` inside attack range, matching HUD nearest-AI state, and at least one attack proof without killing the player.
- `Tools\Open-FWMVPCombatGaragePIE.ps1 -StartPIE -DebugPresentationEvidence` writes a `*_state-evidence.jsonl` file for `PIE-20` with DebugSubsystem enabled state, printed actor state fields, actor health, hit-zone field, recent event count/tags, latest event details, and HUD player/AI debug state after normal weapon/damage gameplay events.
- `Tools\Open-FWMVPCombatGaragePIE.ps1 -StartPIE -UIActionSafetyEvidence` writes a `*_state-evidence.jsonl` file for `PIE-21` with HUD refresh and `SetHUDState` presentation probes, proving player health, lives, core vehicle health, and match state remain stable with `mutationEventCount=0`.
- `Tools\Open-FWMVPCombatGaragePIE.ps1 -StartPIE -PerformanceSmokeEvidence` writes a `*_state-evidence.jsonl` file for `PIE-22` with default-loop AI/vehicle counts, sample duration, frame cadence diagnostics, severe hitch count, and `severeResponsivenessIssue=false`. The log segment for the same run should contain only bounded performance evidence step logs, not per-frame `LogTemp` spam.
- `Tools\Open-FWMVPCombatGaragePIE.ps1 -StartPIE -FullLoopEvidence` writes a `*_state-evidence.jsonl` file for `PIE-23` with spawn baseline, AI weapon damage, core vehicle enter/drive/exit distance, core vehicle life penalty, respawn continuation, final core vehicle destruction, HUD lives reaching `0`, and `Match.Ended`.
...
- Vehicle enters `State.Vehicle.Destroyed` when health reaches zero; `PIE-13` evidence should record alive health/state before lethal damage, then destroyed state and dead health afterward.
- Vehicle destroyed event fires once; `PIE-13` evidence should record delegate and gameplay event counts staying at `1` after a repeated destroy attempt.
- First player core vehicle is registered in `AFWPlayerState`; commandlet coverage already verifies this from the actual map fixture, and `PIE-14` evidence should record `isCoreVehicle=true`, PlayerState/HUD lives decreasing by the configured core vehicle penalty, core vehicle health reaching `0`, and no repeated life penalty after a repeated destroy attempt.
...
- AI pursues when outside attack range; `PIE-18` evidence should record the target AI at a distance greater than attack range with both actor state and HUD nearest-AI state equal to `State.AI.Alert`.
- AI attacks when inside attack range; `PIE-18` evidence should record the same AI inside attack range with both actor state and HUD nearest-AI state equal to `State.AI.AttackOnFoot`, plus weapon/damage event proof or player health reduction while the player remains alive.
...
- AI state appears in Debug output; commandlet coverage verifies AI state transitions, and `PIE-20` Debug presentation evidence records actor/HUD state plus the recent gameplay event buffer.
...
- Lives reaching zero ends the match; `PIE-16` evidence should record end-match mode starting at PlayerState/HUD lives `1`, core vehicle destruction dropping both values to `0`, `Match.Ended` staying stable after a repeated destroy attempt, and `respawnEventCount=0`.
...
- Debug event log shows recent gameplay events; `PIE-20` evidence should include `Event.Weapon.Fired` and `Event.Damage.Applied` in `recentEventTags`.
- Debug actor state shows health, state, and hit zone data; `PIE-20` evidence should include `actorState`, `actorHealth`, and the `hitZone` field from the same actor printed by DebugSubsystem.
- Debug visual output matches the event buffer covered by `FWMVPRuntimeVerify`; `PIE-20` evidence should keep `debugEnabled=true` and a non-empty latest event after the buffer-visible step.
- No UI action directly changes health, lives, vehicle destruction, or match end state; `PIE-21` evidence should keep `stateStable=true` and `mutationEventCount=0` across baseline, HUD refresh probe, and final stable-state observation.

## Performance Smoke

- Test map remains responsive with the default AI count.
- No obvious per-frame log spam; `PIE-22` evidence should keep the performance smoke log segment bounded to the expected start/step messages.
- No unbounded event log growth.
- No unexpected Tick usage outside intentional systems; `PIE-22` evidence should record a stable default loop with no severe hitches, while source review should confirm Tick remains limited to intentional PlayerController HUD/evidence, AI controller, vehicle, and commandlet-only verification paths.
