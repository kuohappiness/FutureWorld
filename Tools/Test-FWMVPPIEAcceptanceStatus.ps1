param(
    [string]$OutputRoot = (Join-Path (Split-Path -Parent $PSScriptRoot) "Saved\PIEAcceptanceStatusSynthetic\Run-$PID"),
    [switch]$KeepOutput
)
...
$statusScript = Join-Path $PSScriptRoot "Get-FWMVPPIEAcceptanceStatus.ps1"
...
function New-SyntheticEvidence {
    ...
    $reportLines.Add("- Notes or issue links: None")
    $reportLines.Add("")
    for ($index = 0; $index -lt 24; $index++) {
        $mark = if ($CompleteReport) { "x" } else { " " }
        $reportLines.Add(("- [{0}] PIE-{1:D2} Synthetic checked item." -f $mark, $index))
    }
    $reportLines.Add("")
    if ($CompleteReport) {
        $reportLines.Add("- Overall status: Pass")
        $reportLines.Add("- Blocking issues: None")
        $reportLines.Add("- Non-blocking notes: None")
    }
    else {
...
function Invoke-Status {
    ...
    try {
        $output = & powershell -NoProfile -ExecutionPolicy Bypass -File $statusScript -ReportPath $reportPath -Tester "CodexSynthetic" *>&1
    ...
    if ($ExpectComplete -and $exitCode -ne 0) {
        Write-Host $outputText
        throw "Status script was expected to report completion."
    }
    if (-not $ExpectComplete -and $exitCode -eq 0) {
        Write-Host $outputText
        throw "Status script was expected to report incomplete acceptance."
    }
    foreach ($text in $ExpectedText) {
        if ($outputText -notmatch [regex]::Escape($text)) {
            Write-Host $outputText
            throw "Status output did not include expected text: $text"
        }
    }
}
...
try {
    ...
    Invoke-Status -ExpectedText @("[WARN] FW MVP PIE acceptance is not complete.", "After manual PIE is truly complete, finalize the report:", "Final validator:")

    New-SyntheticEvidence -CompleteReport
    Invoke-Status -ExpectComplete -ExpectedText @("[OK] FW MVP PIE acceptance is complete.", "Run final readiness gate before commit/push:")

    Remove-Item -LiteralPath $screenshotPath -Force
    Invoke-Status -ExpectedText @("Open manual PIE and capture at least one PNG/JPEG/MP4/MOV/WebM evidence file", "Open-FWMVPCombatGaragePIE.ps1", "Invoke-FWMVPPIEPlayEvidenceCapture.ps1", "Capture-FWMVPPIEEvidenceScreenshot.ps1")

    Write-Host "[OK] FW MVP PIE acceptance status self-test completed." -ForegroundColor Green
