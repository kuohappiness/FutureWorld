param(
    ...
    [string]$OutputPrefix = ("FW_MVP_PIE_CameraEvidence_{0}" -f (Get-Date -Format "yyyyMMdd-HHmmss")),
...
if ($DryRun) {
    Write-Host "[OK] FW MVP PIE camera evidence dry-run passed." -ForegroundColor Green
...
if (-not $editorProcess) {
    throw "UnrealEditor is not running. Start PIE before camera evidence capture."
}

if (-not ("FWCameraEvidenceWin32" -as [type])) {
Add-Type @'
using System;
using System.Runtime.InteropServices;
public class FWCameraEvidenceWin32 {
...
function Get-EditorWindowHandle {
    ...
    [FWCameraEvidenceWin32]::EnumWindows({
        param($hWnd, $lParam)

        $windowPid = [uint32]0
        [void][FWCameraEvidenceWin32]::GetWindowThreadProcessId($hWnd, [ref]$windowPid)
        if ($windowPid -ne $targetPid) {
            return $true
        }

        $rect = New-Object FWCameraEvidenceWin32+RECT
        if (-not [FWCameraEvidenceWin32]::GetWindowRect($hWnd, [ref]$rect)) {
            return $true
        }
        ...
        [void][FWCameraEvidenceWin32]::GetClassName($hWnd, $classSb, $classSb.Capacity)
        [void][FWCameraEvidenceWin32]::GetWindowText($hWnd, $titleSb, $titleSb.Capacity)
        $className = $classSb.ToString()
        $title = $titleSb.ToString()
        if ($className -eq "ConsoleWindowClass" -or $title -like "*.exe*") {
            return $true
        }
        if (-not [FWCameraEvidenceWin32]::IsWindowVisible($hWnd) -and $className -ne "UnrealWindow") {
            return $true
        }
...
function Set-EditorWindowForeground {
    ...
    [void][FWCameraEvidenceWin32]::ShowWindowAsync($WindowHandle, 3)
    [void][FWCameraEvidenceWin32]::SetWindowPos($WindowHandle, $hwndTopMost, 0, 0, 0, 0, $flags)
    [void][FWCameraEvidenceWin32]::BringWindowToTop($WindowHandle)
    [void][FWCameraEvidenceWin32]::SetForegroundWindow($WindowHandle)
    Start-Sleep -Milliseconds 250
    [void][FWCameraEvidenceWin32]::SetWindowPos($WindowHandle, $hwndNoTopMost, 0, 0, 0, 0, $flags)
}
...
function Invoke-KeyTap {
    ...
    $scanCode = [byte]([FWCameraEvidenceWin32]::MapVirtualKey($VirtualKey, 0) -band 0xff)
    [FWCameraEvidenceWin32]::keybd_event($VirtualKey, $scanCode, 0, [UIntPtr]::Zero)
    Start-Sleep -Milliseconds $HoldMilliseconds
    [FWCameraEvidenceWin32]::keybd_event($VirtualKey, $scanCode, 0x0002, [UIntPtr]::Zero)
}
...
function Invoke-MouseClick {
    ...
    [void][FWCameraEvidenceWin32]::SetCursorPos($X, $Y)
    Start-Sleep -Milliseconds 120
    [FWCameraEvidenceWin32]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
    Start-Sleep -Milliseconds 80
    [FWCameraEvidenceWin32]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
}
...
function Capture-Step {
    ...
    if (-not $?) {
        throw "Screenshot capture failed for camera step: $Suffix"
    }
}
...
$rect = New-Object FWCameraEvidenceWin32+RECT
if (-not [FWCameraEvidenceWin32]::GetWindowRect($mainWindow, [ref]$rect)) {
...
Capture-Step "00-third-person"

Invoke-KeyTap -VirtualKey 0x56
Start-Sleep -Milliseconds $StepDelayMilliseconds
Capture-Step "01-first-person"

Invoke-KeyTap -VirtualKey 0x56
Start-Sleep -Milliseconds $StepDelayMilliseconds
Capture-Step "02-third-person-restored"

Write-Host "[OK] FW MVP PIE camera evidence capture completed." -ForegroundColor Green
